...
class ResNet(nn.Module):
    maml = False #Default
    def __init__(self,block,list_of_num_layers, list_of_out_dims, num_views, flatten = True):
    ...
    def forward(self,x):
        ...
        if self.num_views:
#             if flatten:
#                 y = y.view((int(x.shape[0]/self.num_views),self.num_views,y.shape[-1]))#(8,12,512)
#             else:
#                 y = y.view((int(x.shape[0]/self.num_views),self.num_views,y.shape[-3],y.shape[-2],y.shape[-1]))
            y = y.view((int(x.shape[0]/self.num_views),self.num_views,y.shape[-1]))#(-1,num_views,512)
...
def ResNet18( num_views, flatten = True):
    return ResNet(SimpleBlock, [2,2,2,2],[64,128,256,512], num_views, flatten)
