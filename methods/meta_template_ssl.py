...
class MetaTemplate(nn.Module):
    def __init__(self, model_func, num_views, n_way, n_support, use_bn, tracking=False, change_way = True):
        ...
        if isinstance(model_func,str):
            ...
            if self.num_views:
                self.feature    = model_func(self.num_views,flatten=True)
            else:
                self.feature    = model_func(self.num_views)      
        self.feat_dim   = self.feature.final_feat_dim
    ...
    def parse_feature(self,x,is_feature):
        ...
        if is_feature:
            ...
            if self.num_views:
                x = x.contiguous().view( self.n_way * (self.n_support + self.n_query) * self.num_views, *x.size()[3:])
            else:
