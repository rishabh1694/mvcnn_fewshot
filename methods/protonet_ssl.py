...
from methods.meta_template_ssl import MetaTemplate
from itertools import cycle
# from model_resnet import *

class ProtoNet(MetaTemplate):
    def __init__(self, model_func, num_views, n_way, n_support, jigsaw=False, lbda=0.0, rotation=False, use_bn=True):
        super(ProtoNet, self).__init__( model_func, num_views,  n_way, n_support, use_bn)
    ...
    def set_forward_unlabel(self, patches=None, patches_label=None):
        if len(patches.size()) == 6:
            Way,S,T,C,H,W = patches.size()#torch.Size([5, 21, 9, 3, 64, 64])
            B = Way*S
        elif len(patches.size()) == 5:
            B,T,C,H,W = patches.size()#torch.Size([5, 21, 9, 3, 64, 64])
        if self.jigsaw:
#             print(patches.size())
            patches = patches.view(B*T,C,H,W).cuda()#torch.Size([945, 3, 64, 64])
#             print(patches.size())
            patch_feat = self.feature(patches)#torch.Size([675, 512])
#             print(patch_feat.size())
    ...
    def train_loop(self, epoch, train_loader, optimizer):
        ...
        for i, inputs in enumerate(train_loader):
            ...
            if self.jigsaw:
#                 print(inputs[2].size(), inputs[3].size())
                loss_jigsaw, acc_jigsaw = self.set_forward_loss_unlabel(inputs[2], inputs[3])# torch.Size([5, 21, 9, 3, 64, 64]), torch.Size([5, 21])
